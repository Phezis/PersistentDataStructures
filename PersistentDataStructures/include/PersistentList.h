...
namespace pds
{
	...
	public:
		...
		list_node(int version, T value, std::shared_ptr<list_fat_node<T>> prev,
	...
	class version_node
	{
		int m_version;
		int m_size;
		std::shared_ptr<version_node<T>> m_parent;
		std::shared_ptr<version_node<T>> m_child;
	...
	public:
		...
		std::shared_ptr<version_node<T>> get_parent()
		{
			return m_parent;
		}
		...
		std::shared_ptr<version_node<T>> get_child()
		{
			return m_child;
		}

		void set_child(std::shared_ptr<version_node<T>> child)
		{
			m_child = child;
		}

		version_node(int version, int size, std::shared_ptr<list_fat_node<T>> front,
		             std::shared_ptr<list_fat_node<T>> back,
		             std::shared_ptr<version_node<T>> parent)
		{
			m_version = version;
			m_size = size;
			m_front = front;
	...
	public:
		std::vector<std::shared_ptr<list_node<T>>> m_nodes;

		std::vector<std::shared_ptr<list_node<T>>>& get_nodes()
		{
			return m_nodes;
		}

		list_fat_node() : m_nodes(std::vector<std::shared_ptr<list_node<T>>>())
		{
		}

		list_fat_node(std::shared_ptr<list_node<T>> node) : m_nodes(std::vector<std::shared_ptr<list_node<T>>>{node})
		{
		}

		void add_node(std::shared_ptr<list_node<T>> n)
		{
			m_nodes.push_back(n);
		}

		std::shared_ptr<list_node<T>> find_node(std::shared_ptr<version_node<T>> version_node)
		{
		...
		static std::shared_ptr<list_fat_node<T>> update_next(std::shared_ptr<list_fat_node<T>> fat_node,
		                                                     ...
		                                                     std::shared_ptr<version_node<T>> version_node)
		{
			auto found_node = fat_node->find_node(version_node);
			auto new_node = std::make_shared<list_node<T>>(version_node->get_version(), found_node->get_value(),
	...
	class list_const_iterator
	{
		...
		std::shared_ptr<version_node<T>> m_root;
	...
	public:
		...
		list_const_iterator(std::shared_ptr<list_fat_node<T>> node,
		                    std::shared_ptr<version_node<T>> root)
		{
			m_node = node;
			m_root = root;
		}
	...
	class persistent_linked_list
	{
		...
		persistent_linked_list(std::shared_ptr<int> version, std::shared_ptr<version_node<T>> root)
		{
		...
		persistent_linked_list<T> remove_by_node(std::shared_ptr<list_fat_node<T>> fat_node)
		{
			...
			{
				auto newV = std::make_shared<version_node<T>>(++(*m_versionPtr), m_root->size() - 1,
				                                              std::make_shared<list_fat_node<T>>(),
				                                              std::make_shared<list_fat_node<T>>(), m_root);
				return persistent_linked_list<T>(m_versionPtr, newV);
			}

			auto new_version = std::make_shared<version_node<T>>(++(*m_versionPtr), m_root->size() - 1, m_root->front(),
			                                                     m_root->back(),
			                                                     m_root);
		...
		{
			auto v = ++(*m_versionPtr);
			auto new_node = std::make_shared<list_node<T>>(v, value, std::shared_ptr<list_fat_node<T>>(nullptr),
			                                               std::shared_ptr<list_fat_node<T>>(nullptr));
			auto new_fat_node = std::make_shared<list_fat_node<T>>(new_node);

			auto new_version = std::make_shared<version_node<T>>(v, m_root->size() + 1, new_fat_node, new_fat_node,
			                                                     m_root);
	...
	public:
		...
		persistent_linked_list(): m_versionPtr(std::make_shared<int>(0)),
		                          m_root(std::make_shared<version_node<T>>(
			                          ++(*m_versionPtr), 0, std::make_shared<list_fat_node<T>>(),
			                          std::make_shared<list_fat_node<T>>(),
			                          std::shared_ptr<version_node<T>>(nullptr)))
		{
		}
		...
		persistent_linked_list(InputIterator first, InputIterator last)
		{
			...
			auto prev_node = std::shared_ptr<list_node<T>>(nullptr);
			...
			{
				size++;
				auto new_node = std::make_shared<list_node<T>>(v, *first, prev_fat,
				                                               std::shared_ptr<list_fat_node<T>>(nullptr));
			...
			m_root = std::make_shared<version_node<T>>(v, size, front, prev_fat,
			                                           std::shared_ptr<version_node<T>>(nullptr));
		}
		...
		persistent_linked_list<T> insert(int index, T value)
		{
			...
			if (front->is_empty())
			{
				return persistent_linked_list<T>(m_versionPtr,
				                                 std::make_shared<version_node<T>>(
					                                 v, m_root->size() + 1, new_fat_node, new_fat_node, m_root));
			}

			auto newVersion = std::make_shared<version_node<T>>(v, m_root->size() + 1, m_root->front(), m_root->back(),
			                                                    m_root);
			if (index == 0)
			{
		...
		persistent_linked_list<T> set(int index, T value)
		{
			...
			auto v = ++(*m_versionPtr);
			auto newNode
				= std::make_shared<list_node<T>>(v, value, foundNode->get_prev(), foundNode->get_next());
			auto newVersion = std::make_shared<version_node<T>>(v, m_root->size(), m_root->front(), m_root->back(),
			                                                    m_root);
		...
		persistent_linked_list<T> push_back(T value)
		{
			...
			auto v = ++(*m_versionPtr);
			auto newNode = std::make_shared<list_node<T>>(v, value, std::shared_ptr<list_fat_node<T>>(nullptr),
			                                              std::shared_ptr<list_fat_node<T>>(nullptr));
			auto newFatNode = std::make_shared<list_fat_node<T>>(newNode);

			auto newVersion = std::make_shared<version_node<T>>(v, m_root->size() + 1, m_root->front(), newFatNode,
			                                                    m_root);
		...
		persistent_linked_list<T> push_front(T value)
		{
			...
			auto newVersion = std::make_shared<version_node<T>>(v, m_root->size() + 1, newFatNode, m_root->back(),
			                                                    m_root);
		...
		persistent_linked_list<T> undo()
		{
			...
			auto new_root = std::make_shared<version_node<T>>(p->get_version(), p->size(), p->front(), p->back(),
			                                                  p->get_parent());
